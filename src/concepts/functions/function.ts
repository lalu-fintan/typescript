import { userDetail } from "../../reuseable_data/user.type";
...
function printPersonName1(name: string, age: number): void {
...
function printPersonName2(name: string = "lalu", age: number): void {
  console.log(name, age);
}
...
const fullName: string = getFullName2(
...
function getFullName4(person: userDetail): string {
  const fullName = person.firstName + " " + person.lastName;
  return fullName;
}

const name: string = getFullName4(user);
console.log(name);
...
function getPersonDetail(user: userDetail): userDetail {
  user.firstName = "naveen";
  return user;
}

const userDetails: userDetail = getPersonDetail(user);
console.log(userDetails);

// array

function newName1(...names: string[]): string[] {
  return names;
}
